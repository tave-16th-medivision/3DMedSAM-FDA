"""
Usage example:
python train_fft.py --data colon --snapshot_path "/workspace/weights/" --data_prefix "/workspace/dataset/Task10_Colon/" --max_epoch 200 
"""
...
def main():
    ...
    loss_cal = DiceCELoss(include_background=False, softmax=True, to_onehot_y=True, lambda_dice=0.5, lambda_ce=0.5, 
                          smooth_nr=1e-5, smooth_dr=1e-5) # add : 안정화 옵션 
    best_loss = np.inf
    ...
    for epoch_num in range(args.max_epoch):
        ...
        for idx, (img, seg, spacing) in enumerate(train_data):
            ###### add : 종양이 없는 빈 데이터는 학습에서 제외 ######
            if seg.sum() <= 0:
                logger.info(f"Skipping empty mask at iter {idx}") 
                continue
            # 입력 데이터 체크
            if torch.isnan(img).any() or torch.isinf(img).any():
                print(f"Skipping iter {idx}: Input image contains NaN/Inf")
                continue
            ################################################
            print('seg: ', seg.sum())
            ...
            feature_list = [f.detach() for f in feature_list]
